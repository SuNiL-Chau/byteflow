...
import { createReadStream, createWriteStream, rmSync, writeFileSync, readFileSync } from 'node:fs';
import { join } from 'node:path';
import { tmpdir } from 'node:os';

import { fromNode, toNode, fromWeb, text } from '../dist/index.esm.js';
...
describe('Integration Tests: File and HTTP Streaming', () => {
    ...
    before(async () => {
        tempFilePath = join(tmpdir(), 'better-streams-test-in.txt');
        outFilePath = join(tmpdir(), 'better-streams-test-out.txt');
